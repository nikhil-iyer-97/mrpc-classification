...
def train(args, train_dataset, model, tokenizer):
    ...
    tot_batch_cnt = 0
    lr_cos = np.cos(np.linspace(0, 1.57, 920)) * 2e-5
    lr_constant = 2e-5
    lr_decay = np.linspace(0, 2e-5, 459)
    lr_decay = lr_decay[::-1]
    print("Explore = " , 459)
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            """
            if global_step < 459:
                update_lr(optimizer, lr_constant)
            else:
                update_lr(optimizer, lr_decay[tot_batch_cnt - 459])
            """
            update_lr(optimizer, lr_cos[global_step])
            tot_batch_cnt += 1
